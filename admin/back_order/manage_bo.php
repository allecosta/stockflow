...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title"><?= isset($id) ? "Detalhes do Pedido de Compra - ".$po_code : 'Criar Nova Ordem de Compra' ?></h4>
    </div>
    <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="supplier_id" id="supplier_id" class="custom-select select2">
                                <option <?php echo !isset($supplierID) ? 'selected' : '' ?> disabled></option>
                                ...
                                while ($row = $supplier->fetch_assoc()): ?>
                                    <option value="<?= $row['id'] ?>" <?= isset($supplierID) && $supplierID == $row['id'] ? "selected" : "" ?> ><?= $row['name'] ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                    </div>
...
<script>
    ...
    $(function() {
        ...
        $('#item_id').select2({
            placeholder:"Por favor, selecione o fornecedor primeiro",
            width:'resolve',
        })
        ...
        $('#supplier_id').change(function() {
            ...
            if (!!items[supplierID]) {
                ...
                list_item.then(function(){
                    $('#item_id').select2({
                        placeholder:"Por favor, selecione o item aqui",
                        width:'resolve',
                    })
                })
            } else {
        ...
        $('#add_to_list').click(function() {
            ...
            if ($('table#list tbody').find('tr[data-id="'+ item +'"]').length > 0) {
                alert_toast('O item já existe na lista.','erro');
                return false;
            }
        ...
        $('#po-form').submit(function(e) {
			...
			$.ajax({
				...
				error:err=> {
					console.log(err)
					alert_toast("Ocorreu um erro",'erro');
					end_loader();
				},
